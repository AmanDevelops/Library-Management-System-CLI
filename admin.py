...
if ch == "1":
	while True:
		...
		while True:
			id,binfo = book_check()
			mycur.execute(f"SELECT * FROM BOOKS WHERE ID = '{id}' AND ISSUED_TO = 0")
	...
	print("Do You Want To Add Book Through ISBN Number? (y/n)")
	isisbn = input("")
	if isisbn in ("n", "nn", "not"):
		while True:
			mycur.execute("SELECT * FROM BOOKS")
			print("The Last Book ID was", mycur.fetchall()[-1][0])
			while True:
		...
		while True:
			...
			if metadata != {}:
				...
				while True:
					...
					if mycur.fetchone() == None:
						break
				mycur.execute(f"INSERT INTO BOOKS (ID,TITLE,AUTHOR,PRICE) VALUES ({id}, '{metadata['Title']}','{metadata['Publisher']}',{input('Enter Price')})")
				print(id,metadata['Title'],metadata['Publisher'])
				mydb.commit()
				print("[green]Book Added Successfully! ")
				if input("Do You Want to Add More ? [red](y/n)[/]").lower() in ("n", "nn", "not"):
					break
